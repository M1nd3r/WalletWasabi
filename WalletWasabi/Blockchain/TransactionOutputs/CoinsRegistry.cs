...
public class CoinsRegistry : ICoinsView
{
	...
	private bool TryAddNoLock(SmartCoin coin)
	{
		...
		var added = InitializeSmartCoinData(coin);

		if (added)
		{
			UpdateCoinMappings(coin);
		}
		UpdateTransactionAmountNoLock(coin.TransactionId, coin.Amount);
		return added;
	}

	private bool InitializeSmartCoinData(SmartCoin coin)
	{
	...
	private void UpdateCoinMappings(SmartCoin coin)
	{
